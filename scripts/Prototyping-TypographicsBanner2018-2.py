...
BANNER_DATA = [
    dict(filePath='ProtypingDDSBanner-EvenBrightSmall.gif', w=460, h=230, title1=TITLE1, title2=TITLE2, imagePaths=IMAGES1, labelSize=18, titleSize=48),
]
...
def makeBanner(bd):
    ...
    for pn in range(1, FRAMES+1):
        ...
        bs = context.newString('Typographics', style=dict(font=fontRegular.path, fontSize=w/24, xTextAlign=CENTER, textFill=1, rLeading=1.05))
        tw, th = bs.textSize()
        newTextBox(bs, parent=page, h=th+P/2, w=ww25, padding=(P/4,P/3,0,P/3), fill=(1,0,0,0.8), conditions=(Left2LeftSide(), Top2TopSide()))
        ...
        bs += context.newString('Petr van Blokland April 11-13', style=dict(font=fontRegular.path, textFill=C, fontSize=w/17.5, rTracking=0.032, rLeading=1.6))
