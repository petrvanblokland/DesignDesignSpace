...
from pagebot.elements.web.barebonesslider.siteelements import *
#from pagebot.themes import FreshAndShiny as Theme
#from pagebot.themes import IntoTheWoods as Theme
#from pagebot.themes import FairyTales as Theme
from pagebot.themes import BusinessAsUsual as Theme

from css.nanostyle_css import cssPy

SITE_NAME = 'DesignDesign.Space' # Also used as logo

MD_PATH = 'Program2019-01-23.md'
...
def makeTemplates(doc):
    ...
    home = Template('Home')
    wrapper = Wrapper(cssId='Wrapper', parent=home)

    # Header
    header = Header(parent=wrapper)
    logo = Logo(cssId='Logo', parent=header)
    bs = doc.context.newString(SITE_NAME)
    TextBox(bs=bs, parent=logo)
    BurgerButton(parent=header)

    # Responsive conditional menus
    Navigation(cssId='Navigation', cssClass="topnav", parent=header)
    MobileMenu(cssId='MobileMenu', parent=header)

    # Banner on full width
    Banner(cssId='Banner', parent=header)
    ...
    BareBonesSlideShow(h=300, w=300, cssId='SlideShow', parent=group, duration=0.7,
        ...
        easing=CSS_EASE, frameDuration=4, carousel=2,
        pauseOnHit=True, randomPlay=False)
    BareBonesSlideSide(cssId='SlideSide', parent=group)

    # Content
    for m in range(NUM_CONTENT):
        Introduction(cssId='Introduction%d' % m, parent=wrapper)
        content = Content(cssId='Content%d' % m, parent=wrapper)
        Main(cssId='Main%d' % m, parent=content)
        for n in range(NUM_SIDES):
            Side(cssId='Side%d%d' % (m, n), parent=content)
    ...
    wrapper = Wrapper(cssId='Wrapper', parent=default)
...
def makeSite(styles, viewId):
    ...
    # Generate css by mapping theme.mood on cssPth 
